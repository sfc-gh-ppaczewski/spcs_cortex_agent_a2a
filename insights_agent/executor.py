"""
Executor module for Snowflake Cortex Insights A2A Agent (SPCS only).

Wraps the Snowflake Cortex Agent API. The RETENTION_INSIGHTS_AGENT object
owns the system prompt that enforces the structured report format:
    ## Executive Summary
    ## Key Findings
    ## Recommendations
"""
...
class SnowflakeCortexInsightsExecutor(AgentExecutor):
    """
    A2A executor that interfaces with Snowflake Cortex Agent via SPCS,
    specialised for analytical insights and structured business reports.
    """
    ...
    def __init__(self):
        ...
        print("Snowflake Cortex Insights A2A Agent initialized")
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            print(f"[Insights Agent] Received query: {incoming_text}")
            ...
            print("[Insights Agent] Calling Snowflake Cortex API...")
            ...
            if response.status_code != 200:
                print(f"[Insights Agent] Snowflake API Error {response.status_code}: {response.text}")
                await event_queue.enqueue_event(
            ...
            if "text/event-stream" in content_type:
                print("[Insights Agent] Receiving streaming response from Cortex...")
                final_answer = self._parse_sse_response(response)
            else:
                data = response.json()
                final_answer = "I could not retrieve an insights report from the Cortex Agent."
            ...
            print(f"[Insights Agent] Got response from Cortex ({len(final_answer)} chars)")
            ...
            print(f"[Insights Agent] Execution error: {str(e)}")
            await event_queue.enqueue_event(
                TaskStatus(state=TaskState.failed)
            )
