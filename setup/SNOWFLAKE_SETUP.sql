...
CREATE STAGE IF NOT EXISTS TRAVEL_DEMO.AGENTS.LLM_MODELS
    ENCRYPTION = (TYPE = 'SNOWFLAKE_SSE')
    COMMENT = 'Stores GGUF model files for the SPCS llama.cpp server (uploaded via setup/download_llm_model.ipynb)';
...
-- ============================================================================
-- 10. LLM MODEL DOWNLOAD
-- ============================================================================
--
-- The Qwen2.5-1.5B-Instruct GGUF model (~1 GB) must be uploaded to
-- @TRAVEL_DEMO.AGENTS.LLM_MODELS before deploying TRAVEL_ORCHESTRATOR.
--
-- Use the notebook: setup/download_llm_model.ipynb
--   1. Import the notebook into Snowflake Workspace
--   2. Set the runtime to TRAVEL_DEMO_POOL (Container Runtime)
--   3. Run all cells — the notebook downloads from HuggingFace and uploads
--      directly to @TRAVEL_DEMO.AGENTS.LLM_MODELS
--
-- The SPCS compute pool node has sufficient local disk (~tens of GBs) and
-- direct internet egress — no External Access Integration is required.
-- ============================================================================
